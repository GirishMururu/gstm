...
#define STM_STARTUP(numThreads)                   TxOnce(numThreads)
...
#  define STM_BEGIN(isReadOnly, transId)         do { \
                                            STM_JMPBUF_T STM_JMPBUF; \
                                            int STM_RO_FLAG = isReadOnly; \
                                            sigsetjmp(STM_JMPBUF, 1); \
                                            TxStart(STM_SELF, &STM_JMPBUF, &STM_RO_FLAG, transId); \
                                        } while (0) /* enforce comma */

#define STM_BEGIN_RD(transId)                  STM_BEGIN(1,transId)
#define STM_BEGIN_WR(transId)                  STM_BEGIN(0,transId)
