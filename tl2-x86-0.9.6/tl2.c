...
TxOnce (int nThreads)
{
    ...
    registerUseAfterFreeHandler();
	//Girish
	//TransTime = mcmc_gettime();
#ifdef modelling
	//perm_profile_init(nThreads);
	DEBUG("init trans");
	//trans_profile_init(nThreads);
	mcmc_start(nThreads);
	DEBUG("init trans success");
#else
#ifdef time_analysis
	mcmc_start(nThreads);
#endif
...
#endif
	//Girish
//	TransTime = mcmc_gettime() - TransTime;
//	printf("\n Transaction Time = %f", TransTime);
#ifdef modelling
	//perm_profile_destroy();
	DEBUG("Destroy trans ");
	//trans_profile_destroy();
	mcmc_stop();
	DEBUG("Destroy trans success");
#else
#ifdef time_analysis
	mcmc_stop();
#endif
 ...
 */
void
TxStart (Thread* Self, sigjmp_buf* envPtr, int* ROFlag, int transId)
{
	//Girish
#ifdef modelling
  mcmc_registerTransId(Self->UniqID,transId);
	//perm_log(Self->UniqID);
	mcmc_determine(Self->UniqID);
#endif
#ifdef time_analysis
  mcmc_registerTransId(Self->UniqID,transId);
	int start = 1;
	mcmc_timeanalyse(Self->UniqID,start);
#endif
	//Mururu
